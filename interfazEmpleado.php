<!-- INTERFAZ DEL EMPLEADO despues de iniciar sesion-->
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <h1>Sistema Usuario</h1>
    <?php 
        $id= $_REQUEST["idempleado"];
        $nombre= $_REQUEST["nameempleado"];

        // LEYENDO ARCHIVO TXT
        $rutaAd = "archivoEmpleado.txt";
        if(file_exists($rutaAd)){
            $leerAd=fopen("archivoEmpleado.txt","r");
            $flag=true;
            while(!feof($leerAd)){
    ...
    <label>Rol de sesión iniciado: Empleado</label><br>
    <label>Nombre del usuario: <?php echo $nombre ?></label><br>
    <label>Id del usuario: <?php echo $id ?></label><br>
    <!-- Operaciones del empleado -->

    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalUpdateEmpleado" data-bs-whatever="@mdo" id="btnRegistrarmodal">Modificar datos</button>

    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#modalSolicitar" data-bs-whatever="@mdo" id="btnRegistrarmodal">Solicitar prestamo</button>

    <!-- boton para mostrar los datos de los empleados -->
    <?php echo "<a href=tablaPrestamo.php?idempleado=$id&nameempleado=$nombre>" ?> <button type="button" class="btn btn-warning">Tabla de Amortización</button></a>

    <!-- regresar al inicio de sesion -->
    <a href="index.html"><button type="button" class="btn btn-warning">Volver</button></a>


    <!-- Modal para modificar datos admin -->
    <div class="modal fade" id="modalUpdateEmpleado" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered  modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="modificarEmpleado.php" method="post" id="registro-form">
                        <div class="mb-3">
    ...
    <!-- Modal para solicitar prestamo -->
    <div class="modal fade" id="modalSolicitar" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered  modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Solicitar prestamo: Empleado</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <!-- se usa la funcion de modificarAdmin -->
                    <form <?php echo "action=guardarPrestamo.php?idempleado=$id&nameempleado=$nombre" ?> method="post" id="registro-form">
                        <div class="mb-3">
                            <label for="recipient-name" class="col-form-label">id del empleado:</label>
                            <input type="text" class="form-control" id="idUser" name="idUser" value="<?php echo $claveId ?>" readonly>
                        </div>
                        <div class="mb-3">
                            <input type="text" class="form-control" id="idSal" name="idSal" value="<?php echo $claveSal ?>" style="display:none">
                        </div>
                        <div class="mb-3">
                            <label for="recipient-name" class="col-form-label">Monto de prestamo:</label>
                            <input type="text" class="form-control" id="idMonto" name="idMonto" placeholder="$Monto de prestamo">
                        </div>
                        <div class="mb-3">
                            <label for="recipient-name" class="col-form-label">Número de quincenas:</label>
                            <input type="text" class="form-control" id="idQuicenas" name="idQuincenas" placeholder="Número de quincenas a pagar" required>
                        </div>
                        <button class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                        <button type="submit" class="btn btn-secondary" id="btnguardar">Solicitar prestamo</button>
                    </form>
                </div>
                <div class="modal-footer">
                </div>
    ...
    <!-- <script>
        function alertPrestamo(){
            $monto = document.getElementById("idMonto");
            if($monto >  ($claveSal*3)){
                alert("El prestamo solicitado no te puede ser otorgado");
                console.log("SI");
            }else{
                console.log("NO");
            }
